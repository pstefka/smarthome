...
export TALOS_CLUSTER_NAME=talos-proxmox-cluster
...
- download ISO `wget https://factory.talos.dev/image/${SCHEMATIC_ID}/${TALOS_VERSION}/no-cloud-amd64.iso`
...
- Image Factory - <https://factory.talos.dev/?arch=amd64&platform=nocloud&schematic-id=${SCHEMATIC_ID}&target=cloud&version=${TALOS_VERSION}>
- Initial Installation - <factory.talos.dev/nocloud-installer/${SCHEMATIC_ID}:${TALOS_VERSION}>
- Upgrades - <factory.talos.dev/nocloud-installer/${SCHEMATIC_ID}:${TALOS_VERSION}>
- Local Test Cluster - `talosctl cluster create qemu --schematic-id=${SCHEMATIC_ID} --talos-version=${TALOS_VERSION}`
- PXE booter `docker run --rm --network host ghcr.io/siderolabs/booter:v0.3.0 --talos-version=${TALOS_VERSION} --schematic-id=${SCHEMATIC_ID}`
...
- with QEMU agent support - `talosctl gen config $CLUSTER_NAME https://$CONTROL_PLANE_IP:6443 --output-dir _out --install-image factory.talos.dev/installer/${SCHEMATIC_ID}:${TALOS_VERSION}`

- create control plane node - `talosctl apply-config --insecure --nodes $CONTROL_PLANE_IP --file _out/controlplane.yaml`
- ~~set endpoint - `talosctl --talosconfig=_out/talosconfig config endpoints $CONTROL_PLANE_IP`~~

- create worker node - `talosctl apply-config --insecure --nodes $WORKER_IP --file _out/worker.yaml`
...
- save setup key in `netbird.yaml`
...
- update machine config `talosctl patch mc -p @netbird.yaml`

## Post install

### Running workloads on control plane nodes

- in _out/controlplane.yaml set
...
- apply config `talosctl apply-config --file _out/controlplane.yaml`
...
- Longhorn <https://docs.siderolabs.com/kubernetes-guides/csi/storage#longhorn>
